  ...
  while True:  # until EOF
    ...
    while(True):
      ...
      if(index_next_hadith != -1):
        ...
        for i in range(1,10):
          if(current_chunk.find(convert_numbers.english_to_hindi(hadithCount+i)+ " -") != -1):
            for j in range(i):
              fout.write(str(hadithCount-1) + ' | ' + "MANUAL_CORRECTION_NEEDED_" + str(j + 1) + '\n')
              populateHadithInfo(hadithCount-j+1, hadithCountInBook + j)
              hadithCount += 1
              hadithCountInBook += 1
            break
